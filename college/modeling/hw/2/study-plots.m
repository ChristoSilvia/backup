...
h_req_stdiv = linspace(0, 1.5, n_stdivs); 
b_stdiv = 0.1;


gpa_means = zeros(1,n_stdivs);
major_gpa_means = zeros(1,n_stdivs);
gpa_stdivs = zeros(1,n_stdivs);
major_gpa_stdivs = zeros(1,n_stdivs);

for j = 1:n_stdivs
	for i = 1:n_samples
		noise_h_req= randn(6,1)*h_req_stdiv(j);
	...
	gpa_means(j) = mean(gpas);
	major_gpa_means(j) = mean(major_gpa);
	gpa_stdivs(j) = mean(gpas .^ 2) - mean(gpas) .^ 2;
	major_gpa_stdivs(j) = mean(major_gpa .^ 2) - mean(major_gpa) .^ 2;
end

figure
hold on
plot(h_req_stdiv, gpa_means)
hold off
print("means.png")
clf
...
figure 
hold on
plot(h_req_stdiv, gpa_stdivs)
hold off
print("standard-divs.png")
clf
