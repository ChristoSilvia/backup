...
for j = min_max_stockpile:max_max_stockpile
	for i = min_fixed_delivery:max_fixed_delivery
		...
		if (sum((pareto_points(:,4) > ev) .* (pareto_points(:,5) < sigma)) == 0)
			...
			pareto_points = [ pareto_points(non_eliminated_rows,:); [ order_when_out j i ev sigma] ];
		end
	end
end
...
for j = min_max_stockpile:max_max_stockpile
	for i = min_fixed_delivery:max_fixed_delivery
		...
		if (sum((pareto_points(:,4) > ev) .* (pareto_points(:,5) < sigma)) == 0)
			printf('%d %d %d %f %f is pareto-optimal', order_when_out, j, i, ev, sigma)
			non_eliminated_rows = (pareto_points(:,4) > ev) | (pareto_points(:,5) < sigma);
			pareto_points = [ pareto_points; [ order_when_out j i ev sigma] ];
		end
	end
end
...
csvwrite('pareto-front-1.csv', pareto_points)

print('pareto-front-1.png')
pause
